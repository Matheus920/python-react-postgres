...
import { PaginationParams, Page, Message } from '../types/common';
import { 
  Resource, 
  ResourceCreate, 
  ResourceUpdate, 
...
export const resourcesApi = {
  ...
  async getResources(
    ...
    // Use the URL with trailing slash to avoid redirect
  ...
  async getMyResources(skip = 0, limit = 10): Promise<Page<Resource>> {
    ...
    const response = await axios.get<Page<Resource>>('/resources/me/', { params });
    return response.data;
  },
  ...
  async shareResource(id: number, shareData: ResourceShare): Promise<Message> {
    const response = await axios.post<Message>(`/resources/${id}/share/`, shareData);
    return response.data;
  },
  ...
  async unshareResource(id: number, userId: number): Promise<Message> {
    const response = await axios.delete<Message>(`/resources/${id}/share/${userId}/`);
    return response.data;
  },
};
